...
type aliasToCluster struct {
	aliasToPort map[alias]pair.Pair[*clusterData, port]
...
func (a *aliasToCluster) ReplaceCluster(clusterID resource.ID, rd *ReconcileData) error {
	...
	if !ok {
		for als := range rd.AliasesData() {
			if res := a.aliasToPort[alias(als)]; res.F1 != nil {
	...
	if a.aliasToPort == nil {
		a.aliasToPort = map[alias]pair.Pair[*clusterData, port]{}
	}

	for als, p := range rd.AliasPort {
		a.aliasToPort[alias(als)] = pair.MakePair(got, port(p))
	}
...
func (a *aliasToCluster) ClusterPort(als alias) (resource.ID, port, bool) {
	if ptr := a.aliasToPort[als].F1; ptr != nil {
		return ptr.clusterID, a.aliasToPort[als].F2, true
	}
